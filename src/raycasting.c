...
/*   Updated: 2025/02/06 22:01:23 by nopareti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_step_and_side_dist(t_raycast *raycast, t_game *game)
{
	...
	{
		raycast->step_x = -1;
		raycast->side_dist_x = (game->player_pos_x - raycast->map_x) * raycast->delta_dist_x;
	}
	else
	{
		raycast->step_x = 1;
		raycast->side_dist_x = (raycast->map_x + 1.0 - game->player_pos_x) * raycast->delta_dist_x;
	}
	if (raycast->ray_dir_y < 0)
	{
		raycast->step_y = -1;
		raycast->side_dist_y = (game->player_pos_y - raycast->map_y) * raycast->delta_dist_y;
	}
	else
	{
		raycast->step_y = 1;
		raycast->side_dist_y = (raycast->map_y + 1.0 - game->player_pos_y) * raycast->delta_dist_y;
	}
}
...
void	dda_algorithm(t_raycast *raycast, t_game *game)
{
	while (raycast->hit == 0)
	{
		...
		if (game->map_info.map[raycast->map_x][raycast->map_y] == '1') raycast->hit = 1;
	}
	if (raycast->side == 0)
...
void	set_curr_line_values(t_raycast *raycast, int x, t_game *game)
{
	...
	raycast->delta_dist_x = (raycast->ray_dir_x == 0) ?  1e30 : fabs(1 / raycast->ray_dir_x);
	raycast->delta_dist_y = (raycast->ray_dir_y == 0) ?  1e30 : fabs(1 / raycast->ray_dir_y);
}
...
void	calc_line_height(t_raycast *raycast, t_game *game)
{
	...
	if (raycast->side == 0)
		raycast->wall_x = game->player_pos_y + raycast->wall_dist * raycast->ray_dir_y;
	else
		raycast->wall_x = game->player_pos_x + raycast->wall_dist * raycast->ray_dir_x;
...
void store_wall_pixels(t_game *game, int x, t_raycast *raycast)
{
    ...
    while (y < raycast->draw_end_y && y < game->screen_height)
    {
        ...
        {
            int color = tex->addr[tex->width * tex_y + raycast->tex_x];
